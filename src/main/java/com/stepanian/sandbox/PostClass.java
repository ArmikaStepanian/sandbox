...
public class PostClass {
    public static void main(String[] args) throws IOException {
        String propsFilename = args.length > 0 ? args[0] : "src/main/resources/application.properties";
        Properties props = new Properties();
        try (InputStream in = Files.newInputStream(Paths.get(propsFilename))) {
            props.load(in);
        }
        String urlString = props.remove("site-url").toString();
        Object userAgent = props.remove("user-agent").toString();
